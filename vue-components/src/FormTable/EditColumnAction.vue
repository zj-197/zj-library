...
import {
	EditTable,
	IsInputting,
	__RowCancelLoading__,
	__RowConfirmLoading__,
	__RowDeleteLoading__, getELTableColumnProps,
} from './utils';
    ...
    <template v-slot:header="{column, index}">
      <slot v-if="$slots.header" name="header" :column="column" :index="index"></slot>
		...
		<template v-slot:default="{ row, index }">
			...
			<el-button v-if="!props.isHiddenCancelBtn"
                 type="danger"
                 :text="props.btnIsText"
                 :loading="row[__RowCancelLoading__]" :disabled="row[__RowConfirmLoading__]" @click.stop="handleCancel(index)">{{props.cancelText}}
			</el-button>
		</template>
		<template v-slot:view="{ row, index, column }">
			<el-button type="primary"
                 v-if="!props.isHiddenEditBtn"
                 :text="props.btnIsText"
                 @click="handleEdit(index)">{{props.editText}}</el-button>
